...
function withTimeout(promise, ms = 10000){
  return Promise.race([
    promise,
    new Promise((_, reject) => setTimeout(() => reject(new Error('通信がタイムアウトしました。ネット接続を確認して、もう一度試してください。')), ms)),
  ]);
}
...
async function uploadPhoto(fileInput){
  ...
  try {
    ...
    await withTimeout(ref.put(f), 20000);
    return await withTimeout(ref.getDownloadURL(), 10000);
  } catch (err) {
...
function wireComments(hostEl, parentCol, docId){
  ...
  form.addEventListener('submit', async (e) => {
    ...
    btn.disabled = true;
    try {
      await withTimeout(commentsCol.add({ author, text, createdAt: new Date().toISOString() }));
      form.text.value = '';
    } catch (err) {
...
function wireMemories(){
  ...
  wireAddToggle(grid, addBtn, tpl, async (form, fileInput) => {
    ...
    await withTimeout(col.add({ author, date, subject, caption, imageUrl: imageUrl || null, createdAt: new Date().toISOString() }));
  });
...
function wireEvents(){
  ...
  wireAddToggle(grid, addBtn, tpl, async (form, fileInput) => {
    ...
    await withTimeout(col.add({ author, date, title, note: form.note.value.trim(), photoUrl: photoUrl || null, createdAt: new Date().toISOString() }));
  });
}
...
function wireNews(){
  ...
  wireAddToggle(grid, addBtn, tpl, async (form) => {
    ...
    await withTimeout(col.add({ author, text, createdAt: new Date().toISOString() }));
  });
}
